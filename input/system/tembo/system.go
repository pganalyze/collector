...
func GetSystemState(server *state.Server, logger *util.Logger) (system state.SystemState) {
	...
	if err != nil {
		server.SelfTestMarkSystemStatsError(fmt.Sprintf("error getting CPU info: %v", err))
	...
	if err != nil {
		server.SelfTestMarkSystemStatsError(fmt.Sprintf("error getting memory info: %v", err))
	...
	if err != nil {
		server.SelfTestMarkSystemStatsError(fmt.Sprintf("error getting disk info: %v", err))
