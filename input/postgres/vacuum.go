...
func RunVacuumForQueryRun(ctx context.Context, db *sql.DB, query string, marker string) (result string, err error) {
    err = validateVacuumQuery(query)
    if err != nil {
        return
    }
...
func validateVacuumQuery(query string) error {
    ...
    if parseResult.Stmts[0].Stmt.GetVacuumStmt() == nil {
