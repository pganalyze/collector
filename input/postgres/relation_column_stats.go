...
func GetColumnStats(ctx context.Context, h CollectionHelper, db *sql.DB, dbName string) (state.PostgresColumnStatsMap, error) {
	...
	if h.HelperExists("get_column_stats", nil) {
		if strings.Contains(h.HelperReturnType("get_column_stats", nil), "pg_stats") {
			...
			h.Logger.PrintWarning("Outdated pganalyze.get_column_stats() function detected in database %s."+
			...
			if h.GlobalOpts.TestRun {
				h.SelfTest.MarkDbCollectionAspectError(dbName, state.CollectionAspectColumnStats, "monitoring helper function pganalyze.get_column_stats outdated")
				h.SelfTest.HintDbCollectionAspect(dbName, state.CollectionAspectColumnStats,
			...
			sourceTable = "pganalyze.get_column_stats()"
			h.Logger.PrintVerbose("Found pganalyze.get_column_stats() stats helper")
		...
		if h.GlobalOpts.TestRun {
			if h.Config.SystemType == "heroku" || h.ConnectedAsSuperUser {
				...
				h.SelfTest.HintDbCollectionAspect(dbName, state.CollectionAspectColumnStats, "Please set up"+
				...
				h.Logger.PrintInfo("Warning: Limited access to table column statistics detected in database %s. Please set up"+
