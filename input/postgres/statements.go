...
func GetStatements(ctx context.Context, server *state.Server, logger *util.Logger, db *sql.DB, globalCollectionOpts state.CollectionOpts, postgresVersion state.PostgresVersion, showtext bool, systemType string) (state.PostgresStatementMap, state.PostgresStatementTextMap, state.PostgresStatementStatsMap, error) {
	...
	if err == sql.ErrNoRows {
		...
		if err != nil {
			server.SelfCheck.MarkCollectionAspectError(state.CollectionAspectPgStatStatements, "extension does not exist in database %s and could not be created: %s", server.Config.DbName, err)
	...
	if foundExtMinorVersion >= 9 {
		...
		server.SelfCheck.MarkCollectionAspectError(state.CollectionAspectPgStatStatements, "extension version not supported (1.%d installed, 1.3+ supported)", foundExtMinorVersion)
	...
	if globalCollectionOpts.TestRun && foundExtMinorVersion < extMinorVersion {
		...
		server.SelfCheck.MarkCollectionAspectWarning(state.CollectionAspectPgStatStatements, pgssMsg)
	}
	...
	if statementStatsHelperExists(ctx, db, showtext) {
		...
		if systemType != "heroku" && !connectedAsSuperUser(ctx, db, systemType) && !connectedAsMonitoringRole(ctx, db) && globalCollectionOpts.TestRun {
			server.SelfCheck.MarkCollectionAspectWarning(state.CollectionAspectPgStatStatements, "monitoring user may have insufficient permissions to capture all queries")
	...
	server.SelfCheck.MarkCollectionAspectOk(state.CollectionAspectPgStatStatements)
