...
func ResetStatements(ctx context.Context, h CollectionHelper, db *sql.DB) error {
	var method string
...
func GetStatements(ctx context.Context, h CollectionHelper, db *sql.DB, showtext bool) (state.PostgresStatementMap, state.PostgresStatementTextMap, state.PostgresStatementStatsMap, error) {
	var err error
	...
	if h.PostgresVersion.Numeric >= state.PostgresVersion17 {
		extMinorVersion = 11
	} else if h.PostgresVersion.Numeric >= state.PostgresVersion14 {
		extMinorVersion = 9
	} else if h.PostgresVersion.Numeric >= state.PostgresVersion13 {
		extMinorVersion = 8
	} else {
		extMinorVersion = 3
	}
	...
	if err == sql.ErrNoRows {
		h.Logger.PrintInfo("pg_stat_statements does not exist, trying to create extension...")
		_, err = db.ExecContext(ctx, QueryMarkerSQL+"CREATE EXTENSION IF NOT EXISTS pg_stat_statements SCHEMA public")
		if err != nil {
			h.SelfTest.MarkCollectionAspectError(state.CollectionAspectPgStatStatements, "extension does not exist in database %s and could not be created: %s", h.Config.DbName, err)
			h.Logger.PrintInfo("HINT - if you expect the extension to already be installed, please review the pganalyze documentation: https://pganalyze.com/docs/install/troubleshooting/pg_stat_statements")
			return nil, nil, nil, err
		}
		extSchema = "public"
	...
	if h.PostgresVersion.Numeric >= state.PostgresVersion14 && foundExtMinorVersion < 9 {
		h.SelfTest.MarkCollectionAspectError(state.CollectionAspectPgStatStatements, "extension version (1.%d) is too old, 1.9 or newer is required.", foundExtMinorVersion)
		h.SelfTest.HintCollectionAspect(state.CollectionAspectPgStatStatements, "Update the extension by running `ALTER EXTENSION pg_stat_statements UPDATE`.")
	}
	...
	if foundExtMinorVersion >= 9 {
		...
		h.SelfTest.MarkCollectionAspectError(state.CollectionAspectPgStatStatements, "extension version not supported (1.%d installed, 1.3+ supported)", foundExtMinorVersion)
	...
	if h.GlobalOpts.TestRun && foundExtMinorVersion < extMinorVersion {
		pgssMsg := fmt.Sprintf("extension outdated (1.%d installed, 1.%d available)", foundExtMinorVersion, extMinorVersion)
		h.Logger.PrintInfo("pg_stat_statements %s. To update run `ALTER EXTENSION pg_stat_statements UPDATE`", pgssMsg)
		if extMinorVersion >= 9 {
			...
			h.Logger.PrintError("Outdated pg_stat_statements may cause incorrect query statistics")
			pgssMsg += "; outdated pg_stat_statements may cause incorrect query statistics"
		}
		h.SelfTest.MarkCollectionAspectWarning(state.CollectionAspectPgStatStatements, pgssMsg)
		h.SelfTest.HintCollectionAspect(state.CollectionAspectPgStatStatements, "To update run `ALTER EXTENSION pg_stat_statements UPDATE`")
	}
	...
	if statementStatsHelperExists(ctx, db, showtext) {
		...
		if !showtext {
			...
			h.Logger.PrintVerbose("Found pganalyze.get_stat_statements() stats helper")
	...
	if showtext {
		...
		for idx, length := range queryLengths {
			...
			default:
				fingerprintAndNormalize(h, key, query, statements, statementTextsByFp)
			}
			stats := queryStats[idx]
			if ignoreIOTiming(h.PostgresVersion, query) {
				stats.BlkReadTime = 0
	...
	h.SelfTest.MarkCollectionAspectOk(state.CollectionAspectPgStatStatements)
...
func fingerprintAndNormalize(h CollectionHelper, key state.PostgresStatementKey, text string, statements state.PostgresStatementMap, statementTextsByFp state.PostgresStatementTextMap) {
	if insufficientPrivilege(text) {
		...
		fp := util.FingerprintQuery(text, h.Config.FilterQueryText, -1)
		statements[key] = state.PostgresStatement{Fingerprint: fp}
		_, ok := statementTextsByFp[fp]
		if !ok {
			statementTextsByFp[fp] = util.NormalizeQuery(text, h.Config.FilterQueryText, -1)
		}
	}
}
