...
func GetRelationStats(ctx context.Context, h CollectionHelper, db *sql.DB, currentDatabaseOid state.Oid, ts state.TransientState) (relStats state.PostgresRelationStatsMap, err error) {
	var insertsSinceVacuumField string
	var systemCatalogFilter string

	if h.PostgresVersion.Numeric >= state.PostgresVersion13 {
	...
	relStats, err = handleRelationStatsAux(ctx, h, db, relStats)

	return
}

func GetIndexStats(ctx context.Context, h CollectionHelper, db *sql.DB, currentDatabaseOid state.Oid, ts state.TransientState) (indexStats state.PostgresIndexStatsMap, err error) {
	...
	indexStats, err = handleIndexStatsAux(ctx, h, db, indexStats)
