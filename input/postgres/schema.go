...
func CollectAllSchemas(ctx context.Context, h CollectionHelper, server *state.Server, ps state.PersistedState, ts state.TransientState) (state.PersistedState, state.TransientState, error) {
	...
	for _, dbName := range GetDatabasesToCollect(server.Config, ts.Databases) {
		...
		h.SelfTest.MarkMonitoredDb(dbName)

		collected[dbName] = true
		psNext, tsNext, databaseOid, err := collectOneSchema(ctxSchema, h, server, ps, ts, dbName)
		if err != nil {
			// If the outer context failed, return an error to the caller
			if ctx.Err() != nil {
				h.SelfTest.MarkRemainingDbCollectionAspectError(state.CollectionAspectSchema, err.Error())
				return ps, ts, err
			}
			...
			if ctxSchema.Err() != nil {
				h.Logger.PrintWarning("Failed to collect schema metadata for database %s and all remaining databases: %s", dbName, err)
				h.SelfTest.MarkRemainingDbCollectionAspectError(state.CollectionAspectSchema, err.Error())
				return ps, ts, nil
			}
			warning := "Failed to collect schema metadata for database %s: %s"
			h.SelfTest.MarkDbCollectionAspectError(dbName, state.CollectionAspectSchema, err.Error())
			if h.GlobalOpts.TestRun {
				h.Logger.PrintWarning(warning, dbName, err)
			} else {
				h.Logger.PrintVerbose(warning, dbName, err)
			}
		...
		h.SelfTest.MarkDbCollectionAspectOk(dbName, state.CollectionAspectSchema)
	}
	...
	if relCount := len(ps.Relations); relCount > schemaTableLimit {
		...
		if h.GlobalOpts.TestRun {
			for _, dbName := range h.SelfTest.MonitoredDbs {
				h.SelfTest.MarkDbCollectionAspectError(dbName, state.CollectionAspectSchema, "too many total tables")
				h.SelfTest.HintDbCollectionAspect(dbName, state.CollectionAspectSchema, "Too many total tables: got %d, but only %d can be monitored per server; schema information will not be sent; learn more at %s", relCount, schemaTableLimit, selftest.URLPrinter.Sprint("https://pganalyze.com/docs/collector/settings#schema-filter-settings"))
			}
		}
		h.Logger.PrintWarning("Too many tables: got %d, but only %d can be monitored per server; schema information will not be sent; learn more at https://pganalyze.com/docs/collector/settings#schema-filter-settings", relCount, schemaTableLimit)
	}

	return ps, ts, nil
}

func collectOneSchema(ctx context.Context, h CollectionHelper, server *state.Server, ps state.PersistedState, ts state.TransientState, dbName string) (psOut state.PersistedState, tsOut state.TransientState, databaseOid state.Oid, err error) {
	schemaConnection, err := EstablishConnection(ctx, server, h.Logger, h.GlobalOpts, dbName)
	if err != nil {
	...
	psOut, tsOut, err = collectSchemaData(ctx, h, schemaConnection, ps, ts, databaseOid, server, dbName)
	if err != nil {
		return ps, ts, 0, err
	}
...
func collectSchemaData(ctx context.Context, h CollectionHelper, db *sql.DB, ps state.PersistedState, ts state.TransientState, databaseOid state.Oid, server *state.Server, dbName string) (state.PersistedState, state.TransientState, error) {
	if h.GlobalOpts.CollectPostgresRelations {
		...
		newRelationStats, err := GetRelationStats(ctx, h, db, databaseOid, ts)
		if err != nil {
		...
		newIndexStats, err := GetIndexStats(ctx, h, db, databaseOid, ts)
		if err != nil {
		...
		newColumnStats, err := GetColumnStats(ctx, h, db, dbName)
		if err != nil {
		...
		if h.PostgresVersion.Numeric >= state.PostgresVersion12 {
			newRelationStatsExtended, err := GetRelationStatsExtended(ctx, h, db, dbName)
			if err != nil {
	...
	newTypes, err := GetTypes(ctx, h, db, databaseOid)
	if err != nil {
