...
func GetBufferCache(ctx context.Context, h CollectionHelper, server *state.Server, globalCollectionOpts state.CollectionOpts, channel chan state.BufferCache) {
	start := time.Now()
	bufferCache := make(state.BufferCache)
	db, err := EstablishConnection(ctx, server, h.Logger, globalCollectionOpts, "")
	if err != nil {
		h.Logger.PrintError("GetBufferCache: %s", err)
		channel <- bufferCache
		return
	}
	defer db.Close()
	...
	if sizeGB > server.Config.MaxBufferCacheMonitoringGB {
		if globalCollectionOpts.TestRun {
			h.Logger.PrintWarning("GetBufferCache: skipping collection. To enable, set max_buffer_cache_monitoring_gb to a value over %d", sizeGB)
		}
		channel <- bufferCache
		return
	}
	...
	h.Logger.PrintVerbose("GetBufferCache: finished after %s", time.Since(start))
	channel <- bufferCache
}
