...
SELECT (extract(epoch from COALESCE(backend_start, pg_catalog.pg_postmaster_start_time()))::int::text || pg_catalog.to_char(pid, 'FM0000000'))::bigint,
	...
	backend_start, xact_start, query_start, state_change, COALESCE(wait_event_type, '') = 'Lock' as waiting, backend_xid,
	backend_xmin, wait_event_type, wait_event, backend_type, %s, state, query, %s
FROM %s
...
func GetBackends(ctx context.Context, logger *util.Logger, db *sql.DB, postgresVersion state.PostgresVersion, systemType string, collectPostgresLocks bool) ([]state.PostgresBackend, error) {
	...
	stmt, err := db.PrepareContext(ctx, QueryMarkerSQL+fmt.Sprintf(activitySQL, blockingPidsField, optionalFields, sourceTable))
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		err := rows.Scan(&row.Identity, &row.DatabaseOid, &row.DatabaseName,
			...
			&row.WaitEventType, &row.WaitEvent, &row.BackendType,
			pq.Array(&row.BlockedByPids), &row.State, &row.Query, &row.QueryID)
		if err != nil {
			return nil, err
		}
