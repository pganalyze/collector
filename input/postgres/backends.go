...
func GetBackends(ctx context.Context, h CollectionHelper, db *sql.DB) ([]state.PostgresBackend, error) {
	var blockingPidsField string
	var sourceTable string

	if h.GlobalOpts.CollectPostgresLocks {
	...
	for rows.Next() {
		...
		if h.Config.SystemType == "azure_database" && row.BackendType.Valid {
