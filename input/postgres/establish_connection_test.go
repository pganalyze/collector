...
// Connecting through the pinned address must not change how the hostname is used
// for TLS, so a stale pin cannot silently disable certificate verification.
func TestEstablishConnectionRecoversFromStalePin(t *testing.T) {
	...
	_, err := postgres.EstablishConnection(ctx, server, logger, opts, "")
	if err == nil {
		t.Fatalf("expected connecting to an unroutable pinned address to fail")
	}
	if server.ResolvedHost.Current() != "" {
		t.Errorf("expected the failed connection to drop the pin, still have %s", server.ResolvedHost.Current())
	}

	connection, err := postgres.EstablishConnection(ctx, server, logger, opts, "")
	if err != nil {
		t.Fatalf("expected the next attempt to resolve again and succeed, got: %s", err)
	}
