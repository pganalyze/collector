...
func GetVacuumProgress(ctx context.Context, h CollectionHelper, db *sql.DB) ([]state.PostgresVacuumProgress, error) {
	...
	for idx, row := range vacuums {
		if row.SchemaName == "pg_toast" {
			...
			if err != nil {
				h.Logger.PrintVerbose("Failed to resolve TOAST table \"%s\": %s", row.RelationName, err)
