...
func GetPlans(ctx context.Context, h CollectionHelper, db *sql.DB, showtext bool) (state.PostgresPlanMap, state.PostgresPlanStatsMap, error) {
	var err error

	// Only collect this with Aurora using aurora_stat_plans function for now
	if !h.PostgresVersion.IsAwsAurora {
		return nil, nil, nil
	}
	...
	if err != nil {
		if h.GlobalOpts.TestRun {
			h.Logger.PrintInfo("Function aurora_stat_plans() is not supported because Aurora version is too old. Upgrade to Aurora PostgreSQL version 14.10, 15.5, or later versions to collect query plans and stats.")
		}
		return nil, nil, nil
	}
	...
	if computePlanIdEnabled != "on" {
		if h.GlobalOpts.TestRun {
			h.Logger.PrintInfo("Function aurora_stat_plans() is not supported because aurora_compute_plan_id is turned off. Skipping collecting query plans and stats.")
		}
		return nil, nil, nil
	}
