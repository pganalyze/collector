...
func CollectFull(ctx context.Context, server *state.Server, connection *sql.DB, globalCollectionOpts state.CollectionOpts, logger *util.Logger) (ps state.PersistedState, ts state.TransientState, err error) {
	...
	go func() {
		postgres.GetBufferCache(ctx, h, server, globalCollectionOpts, bufferCacheReady)
	}()
	...
	ts.Statements, ts.StatementTexts, ps.StatementStats, err = postgres.GetStatements(ctx, h, connection, true)
	if err != nil {
		...
		ts.Plans, ps.PlanStats, err = postgres.GetPlans(ctx, h, connection, true)
		if err != nil {
	...
	if config.Features.StatementResetFrequency != 0 && ps.StatementResetCounter >= int(config.Features.StatementResetFrequency) {
		...
		err = postgres.ResetStatements(ctx, h, connection)
		if err != nil {
			...
			_, _, ts.ResetStatementStats, err = postgres.GetStatements(ctx, h, connection, false)
			if err != nil {
	...
	ts.Replication, err = postgres.GetReplication(ctx, h, connection)
	if err != nil {
	...
	ps, ts, err = postgres.GetServerStats(ctx, h, connection, ps, ts)
	if err != nil {
		logger.PrintError("Error collecting Postgres server statistics: %s", err)
		return
	}

	ts.BackendCounts, err = postgres.GetBackendCounts(ctx, h, connection)
	if err != nil {
	...
	ps, ts, err = postgres.CollectAllSchemas(ctx, h, server, ps, ts)
	if err != nil {
